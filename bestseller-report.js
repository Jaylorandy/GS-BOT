...
async function describeStyle(client, style, options) {
  ...
  const prompt = cn ? [
    '你是一名资深服装产品分析师。看图片和文档数据，用简体中文写一条极简笔记（2-3 句，总长不超过 80 字）。',
    '只写最关键的三点：品类与廓形、最突出的设计细节、颜色/图案。',
    '不要分点、不要小标题、不要铺陈，像一句行业速记。',
    '重要：若下方文档成分存在，以其为准，不要与之矛盾。整段必须用简体中文。',
    '',
    `款式名称：${name}`,
    ...
    'You are a senior fashion product analyst. Write a very short English note (2-3 sentences, max 60 words) about the garment images and document data.',
    'Cover only the essentials: garment type & silhouette, the most notable design detail, and colour/print.',
    'No bullet points, no headings, no filler — like a terse industry shorthand note.',
    'IMPORTANT: If the document composition below is present, treat it as authoritative for fibre content. Do NOT contradict it.',
    '',
    `Style name: ${name}`,
...
async function aggregateTrends(client, notes, options) {
  ...
  const prompt = cn ? [
    ...
    '每个章节控制在 2-3 句话内，只保留可归纳的共性结论，并点明有多少款体现该特征。',
    '报告整体保持精简，避免重复表述，不要逐个款式展开描述。',
    ...sectionsCn,
    ...
    'Keep each section to 2-3 sentences, focused on the aggregate takeaway, citing how many styles show each trait.',
    'Keep the whole report concise and non-repetitive — do not describe styles one by one.',
    ...sectionsEn,
